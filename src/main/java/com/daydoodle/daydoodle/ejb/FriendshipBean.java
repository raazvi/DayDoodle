...
public class FriendshipBean {
    ...
    public void createFriendship(String user, String friend) {
        ...
        reverseFriendship.setDateCreated(LocalDate.now());
     ...
     */
    public void removeFriend(Long friendshipId, String username, String friendUsername) {
        log.info("\n Entered removeFriend method \n");

        // Find the friendship in both directions
        TypedQuery<Friendship> query = entityManager.createQuery(
                "SELECT f FROM Friendship f WHERE (f.user.username = :user1 AND f.friend.username = :user2) OR (f.user.username = :user2 AND f.friend.username = :user1)",
                Friendship.class);
        query.setParameter("user1", username);
        query.setParameter("user2", friendUsername);
